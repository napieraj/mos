/*
 * test_stdinq.c — STANDARD INQUIRY decode: the version byte and the
...
TEST(stdinq_version_and_descriptors)
{
    uint8_t buf[96] = {0};
    ...
    EXPECT(mos_internal_stdinq_parse(buf, sizeof buf, &s));
...
TEST(stdinq_additional_length_and_len_both_bound)
{
    uint8_t buf[96] = {0};
    buf[2] = 0x05;                 /* SPC-3 */
    put_be16(&buf[58], 0x00A0);
    ...
    EXPECT(mos_internal_stdinq_parse(buf, 60, &s));
    EXPECT_EQ(s.descriptor_count, 1);
    EXPECT_EQ(s.descriptors[0], 0x00A0);
    return 0;
}

TEST(stdinq_skips_empty_slots_short_and_null)
{
    uint8_t buf[96] = {0};
    ...
    EXPECT(!mos_internal_stdinq_parse(buf, 4, &s));
    EXPECT(!mos_internal_stdinq_parse(NULL, 96, &s));
    EXPECT(!mos_internal_stdinq_parse(buf, 96, NULL));
    return 0;
}

void register_stdinq_tests(void)
{
    RUN(stdinq_version_and_descriptors);
    RUN(stdinq_additional_length_and_len_both_bound);
    RUN(stdinq_skips_empty_slots_short_and_null);
}
